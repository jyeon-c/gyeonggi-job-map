...
| career | string (optional) | 정규화 경력값 |
| edu | string (optional) | 정규화 학력값 |
| employmentType | string (optional) | 정규화 고용형태 |
| jobCategory | string (optional) | 통합 직종 분류 |
| minSalary | int (optional) | 최소 연 환산 임금(만원) |
| keyword | string (optional) | 제목·회사·지역 키워드 |
| source | string (optional) | `public` / `private` 또는 `고용24` / `잡코리아` |
...
직종 분류는 공고 조회·필터에 사용하며, 관리자 통계 화면은 운영 확인에 필요한 출처/지역/경력/학력/고용형태 축을 제공한다.

응답 예시
```json
{
  "total": 460,
  "activeTotal": 336,        // 예시: 기준일 현재 마감 제외 노출 건수
  "expiredTotal": 124,       // 예시: 기준일 현재 마감 공고 수
  "statsDate": "YYYY-MM-DD", // Asia/Seoul 현재일
...
```
`activeTotal`은 `deadline`이 없거나 기준일(`statsDate`, Asia/Seoul 현재일) 이후인 공고 수이며, 지도 화면의 마감 공고 제외 노출 기준과 맞춘다.
...
| Method | URL | 설명 |
|---|---|---|
| GET | `/api/admin/jobs?keyword=&page=0&size=10` | 관리자 공고 목록/검색 |
| GET | `/api/admin/jobs/{id}` | 관리자 공고 상세 |
| POST | `/api/admin/jobs` | 공고 등록 |
| PUT | `/api/admin/jobs/{id}` | 공고 수정 |
| DELETE | `/api/admin/jobs/{id}` | 공고 삭제 |

관리자 CRUD는 운영 DB 기준으로 처리한다. 운영 프로필은 관리자 수정사항 보존을 위해 기본적으로 DB가 비어 있을 때만 `jobs.json` 시드를 적재한다(`JOBMAP_SEED_REPLACE=true` 설정 시 강제 교체).
관리자 공고 등록/수정 화면은 위도·경도를 직접 입력받지 않는다. 서버는 `addressRaw`와 `region`을 기준으로 카카오 REST 지오코딩을 수행하고, 실패하거나 키가 없으면 경기도 시·군 중심점으로 보정한다.
